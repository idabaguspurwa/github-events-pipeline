    ...
    with TaskGroup("transformation_phase", dag=dag) as transformation_group:
        ...
        dbt_transform = NoTemplateKubernetesPodOperator(
            ...
            arguments=[
                ...
                "echo '      +schema: analytics' >> dbt_project.yml && "
                "echo '    production:' >> dbt_project.yml && "
                "echo '      +materialized: table' >> dbt_project.yml && "
                "echo '      +schema: production' >> dbt_project.yml && "
                ...
                "printf '%s\n' '-- Daily activity summary' '{{ config(materialized=\"table\", unique_key=\"activity_date\") }}' 'select' '    date(event_created_at) as activity_date,' '    count(*) as total_events,' '    count(distinct actor_login) as unique_users,' '    count(distinct repo_name) as unique_repositories,' '    count(case when event_type = \"PushEvent\" then 1 end) as push_events,' '    count(case when event_type = \"PullRequestEvent\" then 1 end) as pull_request_events,' '    count(case when event_type = \"IssuesEvent\" then 1 end) as issue_events,' '    count(case when event_type = \"CreateEvent\" then 1 end) as create_events,' '    max(event_created_at) as latest_activity' 'from {{ ref(\"stg_github_events\") }}' 'group by date(event_created_at)' 'order by activity_date desc' > models/production/daily_activity_summary.sql && "
                "echo '👥 Creating user engagement metrics model...' && "
                "printf '%s\n' '-- User engagement metrics' '{{ config(materialized=\"table\", unique_key=\"actor_login\") }}' 'select' '    actor_login,' '    count(*) as total_events,' '    count(distinct repo_name) as repositories_contributed,' '    count(case when event_type = \"PushEvent\" then 1 end) as push_count,' '    count(case when event_type = \"PullRequestEvent\" then 1 end) as pr_count,' '    count(case when event_type = \"IssuesEvent\" then 1 end) as issue_count,' '    min(event_created_at) as first_activity,' '    max(event_created_at) as latest_activity,' '    datediff(day, min(event_created_at), max(event_created_at)) + 1 as active_days,' '    round(count(*) / nullif(datediff(day, min(event_created_at), max(event_created_at)) + 1, 0), 2) as avg_events_per_day' 'from {{ ref(\"stg_github_events\") }}' 'group by actor_login' 'having count(*) >= 5' 'order by total_events desc' > models/production/user_engagement_metrics.sql && "
                "echo '📈 Creating repository popularity rankings model...' && "
                "printf '%s\n' '-- Repository popularity rankings' '{{ config(materialized=\"table\", unique_key=\"repo_name\") }}' 'select' '    repo_name,' '    count(*) as total_activity,' '    count(distinct actor_login) as unique_contributors,' '    count(case when event_type = \"PushEvent\" then 1 end) as push_events,' '    count(case when event_type = \"WatchEvent\" then 1 end) as watch_events,' '    count(case when event_type = \"ForkEvent\" then 1 end) as fork_events,' '    count(case when event_type = \"PullRequestEvent\" then 1 end) as pull_requests,' '    count(case when event_type = \"IssuesEvent\" then 1 end) as issues,' '    max(event_created_at) as latest_activity,' '    min(event_created_at) as first_seen,' '    round(count(*) / count(distinct date(event_created_at)), 2) as avg_daily_activity' 'from {{ ref(\"stg_github_events\") }}' 'group by repo_name' 'having count(*) >= 10' 'order by total_activity desc' > models/production/repository_popularity_rankings.sql && "
