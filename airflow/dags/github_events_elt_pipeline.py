    ...
    with TaskGroup("transformation_phase", dag=dag) as transformation_group:
        ...
        dbt_transform = NoTemplateKubernetesPodOperator(
            ...
            arguments=[
                ...
                "echo '  my_dbt_project:' >> dbt_project.yml && "
                "echo '    staging:' >> dbt_project.yml && "
                "echo '      +materialized: view' >> dbt_project.yml && "
                "echo '    production:' >> dbt_project.yml && "
                ...
                "echo '🏭 Creating production models directory...' && "
                "mkdir -p models/production && "
                "echo '📊 Creating daily activity summary model...' && "
                "echo '-- Daily activity summary' > models/production/daily_activity_summary.sql && "
                "echo '{{ config(materialized=\"table\", unique_key=\"activity_date\") }}' >> models/production/daily_activity_summary.sql && "
                "echo '' >> models/production/daily_activity_summary.sql && "
                "echo 'select' >> models/production/daily_activity_summary.sql && "
                "echo '    date(event_created_at) as activity_date,' >> models/production/daily_activity_summary.sql && "
                "echo '    count(*) as total_events,' >> models/production/daily_activity_summary.sql && "
                "echo '    count(distinct actor_login) as unique_users,' >> models/production/daily_activity_summary.sql && "
                "echo '    count(distinct repo_name) as unique_repositories,' >> models/production/daily_activity_summary.sql && "
                "echo \"    count(case when event_type = 'PushEvent' then 1 end) as push_events,\" >> models/production/daily_activity_summary.sql && "
                "echo \"    count(case when event_type = 'PullRequestEvent' then 1 end) as pull_request_events,\" >> models/production/daily_activity_summary.sql && "
                "echo \"    count(case when event_type = 'IssuesEvent' then 1 end) as issue_events,\" >> models/production/daily_activity_summary.sql && "
                "echo \"    count(case when event_type = 'CreateEvent' then 1 end) as create_events,\" >> models/production/daily_activity_summary.sql && "
                "echo '    max(event_created_at) as latest_activity' >> models/production/daily_activity_summary.sql && "
                "echo 'from {{ ref(\"stg_github_events\") }}' >> models/production/daily_activity_summary.sql && "
                "echo 'group by date(event_created_at)' >> models/production/daily_activity_summary.sql && "
                "echo 'order by activity_date desc' >> models/production/daily_activity_summary.sql && "
                "echo '👥 Creating user engagement metrics model...' && "
                "echo '-- User engagement metrics' > models/production/user_engagement_metrics.sql && "
                "echo '{{ config(materialized=\"table\", unique_key=\"actor_login\") }}' >> models/production/user_engagement_metrics.sql && "
                "echo '' >> models/production/user_engagement_metrics.sql && "
                "echo 'select' >> models/production/user_engagement_metrics.sql && "
                "echo '    actor_login,' >> models/production/user_engagement_metrics.sql && "
                "echo '    count(*) as total_events,' >> models/production/user_engagement_metrics.sql && "
                "echo '    count(distinct repo_name) as repositories_contributed,' >> models/production/user_engagement_metrics.sql && "
                "echo \"    count(case when event_type = 'PushEvent' then 1 end) as push_count,\" >> models/production/user_engagement_metrics.sql && "
                "echo \"    count(case when event_type = 'PullRequestEvent' then 1 end) as pr_count,\" >> models/production/user_engagement_metrics.sql && "
                "echo \"    count(case when event_type = 'IssuesEvent' then 1 end) as issue_count,\" >> models/production/user_engagement_metrics.sql && "
                "echo '    min(event_created_at) as first_activity,' >> models/production/user_engagement_metrics.sql && "
                "echo '    max(event_created_at) as latest_activity,' >> models/production/user_engagement_metrics.sql && "
                "echo '    datediff(day, min(event_created_at), max(event_created_at)) + 1 as active_days,' >> models/production/user_engagement_metrics.sql && "
                "echo '    round(count(*) / nullif(datediff(day, min(event_created_at), max(event_created_at)) + 1, 0), 2) as avg_events_per_day' >> models/production/user_engagement_metrics.sql && "
                "echo 'from {{ ref(\"stg_github_events\") }}' >> models/production/user_engagement_metrics.sql && "
                "echo 'group by actor_login' >> models/production/user_engagement_metrics.sql && "
                "echo 'having count(*) >= 5' >> models/production/user_engagement_metrics.sql && "
                "echo 'order by total_events desc' >> models/production/user_engagement_metrics.sql && "
                "echo '📈 Creating repository popularity rankings model...' && "
                "echo '-- Repository popularity rankings' > models/production/repository_popularity_rankings.sql && "
                "echo '{{ config(materialized=\"table\", unique_key=\"repo_name\") }}' >> models/production/repository_popularity_rankings.sql && "
                "echo '' >> models/production/repository_popularity_rankings.sql && "
                "echo 'select' >> models/production/repository_popularity_rankings.sql && "
                "echo '    repo_name,' >> models/production/repository_popularity_rankings.sql && "
                "echo '    count(*) as total_activity,' >> models/production/repository_popularity_rankings.sql && "
                "echo '    count(distinct actor_login) as unique_contributors,' >> models/production/repository_popularity_rankings.sql && "
                "echo \"    count(case when event_type = 'PushEvent' then 1 end) as push_events,\" >> models/production/repository_popularity_rankings.sql && "
                "echo \"    count(case when event_type = 'WatchEvent' then 1 end) as watch_events,\" >> models/production/repository_popularity_rankings.sql && "
                "echo \"    count(case when event_type = 'ForkEvent' then 1 end) as fork_events,\" >> models/production/repository_popularity_rankings.sql && "
                "echo \"    count(case when event_type = 'PullRequestEvent' then 1 end) as pull_requests,\" >> models/production/repository_popularity_rankings.sql && "
                "echo \"    count(case when event_type = 'IssuesEvent' then 1 end) as issues,\" >> models/production/repository_popularity_rankings.sql && "
                "echo '    max(event_created_at) as latest_activity,' >> models/production/repository_popularity_rankings.sql && "
                "echo '    min(event_created_at) as first_seen,' >> models/production/repository_popularity_rankings.sql && "
                "echo '    round(count(*) / count(distinct date(event_created_at)), 2) as avg_daily_activity' >> models/production/repository_popularity_rankings.sql && "
                "echo 'from {{ ref(\"stg_github_events\") }}' >> models/production/repository_popularity_rankings.sql && "
                "echo 'group by repo_name' >> models/production/repository_popularity_rankings.sql && "
                "echo 'having count(*) >= 10' >> models/production/repository_popularity_rankings.sql && "
                "echo 'order by total_activity desc' >> models/production/repository_popularity_rankings.sql && "
                "echo '📊 Creating hourly trend analysis model...' && "
                "printf '%s\n' '-- Hourly trend analysis' '{{ config(materialized=\"table\", unique_key=[\"activity_hour\", \"event_type\"]) }}' 'select' '    date_trunc(\"hour\", event_created_at) as activity_hour,' '    event_type,' '    count(*) as event_count,' '    count(distinct actor_login) as unique_users,' '    count(distinct repo_name) as unique_repos,' '    extract(hour from event_created_at) as hour_of_day,' '    extract(dow from event_created_at) as day_of_week' 'from {{ ref(\"stg_github_events\") }}' 'group by date_trunc(\"hour\", event_created_at), event_type, extract(hour from event_created_at), extract(dow from event_created_at)' 'order by activity_hour desc, event_count desc' > models/production/hourly_trend_analysis.sql && "
                "echo '🔄 Running dbt transformations on real-time GitHub events data...' && "
                "dbt debug && "
                "dbt run --target prod && "
                "echo '✅ dbt transformations completed successfully with real data including production models'"
            ],
        ...
        dbt_test = NoTemplateKubernetesPodOperator(
            ...
            arguments=[
                ...
                "echo '      - name: raw_events' >> models/staging/sources.yml && "
                "printf '%s\n' '-- Staging model for GitHub events data' 'select' '    v:id::string as event_id,' '    v:type::string as event_type,' '    v:actor:id::int as actor_id,' '    v:actor:login::string as actor_login,' '    v:repo:id::int as repo_id,' '    v:repo:name::string as repo_name,' '    v:created_at::timestamp_ntz as event_created_at,' '    loaded_at' 'from {{ source(\"raw_data\", \"raw_events\") }}' > models/staging/stg_github_events.sql && "
                "echo '🏭 Creating production models for testing...' && "
                "mkdir -p models/production && "
                "echo '📊 Creating daily activity summary model...' && "
                "printf '%s\\n' '-- Daily activity summary' > models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '{{ config(materialized=\"table\", unique_key=\"activity_date\") }}' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' 'select' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '    date(event_created_at) as activity_date,' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '    count(*) as total_events,' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '    count(distinct actor_login) as unique_users,' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '    count(distinct repo_name) as unique_repositories,' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' \"    count(case when event_type = 'PushEvent' then 1 end) as push_events,\" >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' \"    count(case when event_type = 'PullRequestEvent' then 1 end) as pull_request_events,\" >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' \"    count(case when event_type = 'IssuesEvent' then 1 end) as issue_events,\" >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' \"    count(case when event_type = 'CreateEvent' then 1 end) as create_events,\" >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' '    max(event_created_at) as latest_activity' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' 'from {{ ref(\"stg_github_events\") }}' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' 'group by date(event_created_at)' >> models/production/daily_activity_summary.sql && "
                "printf '%s\\n' 'order by activity_date desc' >> models/production/daily_activity_summary.sql && "
                "printf '%s\n' '-- Hourly trend analysis' '{{ config(materialized=\"table\", unique_key=[\"activity_hour\", \"event_type\"]) }}' 'select' '    date_trunc(\"hour\", event_created_at) as activity_hour,' '    event_type,' '    count(*) as event_count,' '    count(distinct actor_login) as unique_users,' '    count(distinct repo_name) as unique_repos,' '    extract(hour from event_created_at) as hour_of_day,' '    extract(dow from event_created_at) as day_of_week' 'from {{ ref(\"stg_github_events\") }}' 'group by date_trunc(\"hour\", event_created_at), event_type, extract(hour from event_created_at), extract(dow from event_created_at)' 'order by activity_hour desc, event_count desc' > models/production/hourly_trend_analysis.sql && "
                "echo '🧪 Running dbt tests on transformed real-time data...' && "
                "dbt test --target prod && "
                "echo '✅ dbt tests completed - data quality verified for staging and production models'"
            ],
            # Use env_from to mount Snowflake credentials from secret
            env_from=[
                k8s.V1EnvFromSource(
        ...
        dbt_docs = NoTemplateKubernetesPodOperator(
            ...
            arguments=[
                ...
                "echo '      - name: raw_events' >> models/staging/sources.yml && "
                "printf '%s\n' '-- Staging model for GitHub events data' 'select' '    v:id::string as event_id,' '    v:type::string as event_type,' '    v:actor:id::int as actor_id,' '    v:actor:login::string as actor_login,' '    v:repo:id::int as repo_id,' '    v:repo:name::string as repo_name,' '    v:created_at::timestamp_ntz as event_created_at,' '    loaded_at' 'from {{ source(\"raw_data\", \"raw_events\") }}' > models/staging/stg_github_events.sql && "
                "echo '🏭 Creating production models for documentation...' && "
                "mkdir -p models/production && "
                "echo '📊 Using pre-built production models...' && "
                "echo '✅ Production models ready for documentation' && "
                "printf '%s\n' '-- Hourly trend analysis' '{{ config(materialized=\"table\", unique_key=[\"activity_hour\", \"event_type\"]) }}' 'select' '    date_trunc(\"hour\", event_created_at) as activity_hour,' '    event_type,' '    count(*) as event_count,' '    count(distinct actor_login) as unique_users,' '    count(distinct repo_name) as unique_repos,' '    extract(hour from event_created_at) as hour_of_day,' '    extract(dow from event_created_at) as day_of_week' 'from {{ ref(\"stg_github_events\") }}' 'group by date_trunc(\"hour\", event_created_at), event_type, extract(hour from event_created_at), extract(dow from event_created_at)' 'order by activity_hour desc, event_count desc' > models/production/hourly_trend_analysis.sql && "
                "echo '📚 Generating dbt documentation for real-time analytics models...' && "
                "dbt docs generate --target prod && "
                "echo '✅ dbt documentation generated for staging and production models'"
            ],
            # Use env_from to mount Snowflake credentials from secret
            env_from=[
                k8s.V1EnvFromSource(
